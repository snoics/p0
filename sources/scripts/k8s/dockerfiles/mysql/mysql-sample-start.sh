...
if [ ! -d "${VOLUME_PATH}/data/mysql" ];
then
  mkdir -p ${VOLUME_PATH}
  tar -zxvf ./data/mysql-data-standard-initializer-1.0.tar -C ${VOLUME_PATH}
  chown -R systemd-coredump:root ${VOLUME_PATH}/config/mysql
  chown -R systemd-coredump:root ${VOLUME_PATH}/data/mysql
  chown -R systemd-coredump:root ${VOLUME_PATH}/logs/mysql
fi
